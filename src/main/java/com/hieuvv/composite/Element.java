...
import static com.hieuvv.function.PreProcessFunction.toLowerCase;
import static com.hieuvv.function.PreProcessFunction.trim;
...
public class Element implements Matchable {
	...
	private static final BiFunction<Match, List<Score>, Score> DEFAULT_ELEMENT_SCORING = ScoringFunction
			.getSimpleAverageScore();

	public Element(ElementType type, String variance, Object value, double weight, double threshold,
			Function<Object, Object> preProcessFunction, Function<Element, Stream<Token>> tokenizerFunction,
		...
		this.similarityMatchFunction = similarityMatchFunction == null ? type.getSimilarityMatchFunction()
				: similarityMatchFunction;
		...
		if (this.preProcessedValue == null) {
			if (this.value instanceof String) {
				...
				setPreProcessedValue(
						getPreProcessFunction().andThen(trim()).andThen(toLowerCase()).apply(this.value.toString()));
			} else {
	...
	/**
	 * This gets the Max number of tokens present between matching Elements. For
	 * Elements that do not have a balanced set of tokens, it can push the score
	 * down.
	 */
	@Override
	...
	public long getUnmatchedChildCount(Matchable other) {
		if (other instanceof Element) {
			Element o = (Element) other;
			long emptyChildren = this.getTokens()
					.filter(token -> token == null || StringUtils.isEmpty(token.getValue().toString())).count();
			long oEmptyChildren = o.getTokens()
					.filter(token -> token == null || StringUtils.isEmpty(token.getValue().toString())).count();
	...
	public boolean equals(Object o) {
		...
		if (value != null ? !value.equals(element.value) : element.value != null)
			return false;
		if (elementClassification != null ? !elementClassification.equals(element.elementClassification)
				: element.elementClassification != null)
			return false;
