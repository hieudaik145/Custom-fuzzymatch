...
/**
 * Matches at element level with aggregated results from token. This uses the
 * ScoringFunction defined at each element to get the aggregated Element score
 * for matched tokens
 */
public class ElementMatch {
	...
	private Stream<Match<Element>> rollupElementScore(Stream<Match<Token>> matchedTokenStream) {
		...
		return groupBy.entrySet().parallelStream().flatMap(
				leftElementEntry -> leftElementEntry.getValue().entrySet().parallelStream().map(rightElementEntry -> {
					List<Score> childScoreList = rightElementEntry.getValue().stream().map(d -> d.getScore())
