...
public interface ScoringFunction extends BiFunction<Match, List<Score>, Score> {
	...
	/**
	 * For all the childScores in a Match object it calculates the average. To get a
	 * balanced average for 2 Match object which do not have same number of
	 * childScores. It gives a score of 0.5 for missing children
	 *
	...
	/**
	 * For all the childScores in a Match object it calculates the average. Average
	 * is calculated with a total of child scored divided by the child count
	 *
	...
	/**
	 * Follows the same rules as "getAverageScore" and in addition applies weights
	 * to children. It can be used for aggregating Elements to Documents, where
	 * weights can be provided at Element level
	 *
	 * @return the scoring function for WeightedAverage
	 */
	static ScoringFunction getWeightedAverageScore() {
		return (match, childScores) -> {
			double numerator = getSumOfWeightedResult(childScores) + getUnmatchedChildScore(match);
			double denominator = getSumOfWeights(childScores) + getChildCount(match) - childScores.size();
			return new Score(numerator / denominator, match);
		};
	}

	/**
	 * Follows the same rules as "getAverageScore", and in addition if more than 1
	 * children match above a score of 0.9, it exponentially increases the overall
	 * score by using a 1.5 exponent
	 *
		...
		return (match, childScores) -> {
			...
			if (perfectMatchedElements.size() > 1 && getSumOfResult(perfectMatchedElements) > 1) {
				double numerator = getExponentiallyIncreasedValue(getSumOfResult(perfectMatchedElements))
						+ getSumOfResult(getNonPerfectMatchedElement(childScores)) + getUnmatchedChildScore(match);

				double denominator = getExponentiallyIncreasedValue(perfectMatchedElements.size())
						+ getChildCount(match) - perfectMatchedElements.size();
				return new Score(numerator / denominator, match);
			} else
	...
	/**
	 * This is the default scoring used to calculate the Document score by
	 * aggregating the child Element scores. This combines the benefits of weights
	 * and exponential increase when calculating the average scores.
	 *
		...
		return (match, childScores) -> {
			...
			if (perfectMatchedElements.size() > 1 && getSumOfWeightedResult(perfectMatchedElements) > 1) {
				...
				double numerator = getExponentiallyIncreasedValue(getSumOfWeightedResult(perfectMatchedElements))
						+ getSumOfWeightedResult(notPerfectMachedElements) + getUnmatchedChildScore(match);

				double denominator = getExponentiallyIncreasedValue(getSumOfWeights(perfectMatchedElements))
						+ getSumOfWeights(notPerfectMachedElements) + getChildCount(match) - childScores.size();
				return new Score(numerator / denominator, match);
			} else
	...
	/**
	 * This scoring assumes the child scores are binary (1.0 or 0.0). Uses the
	 * jaccard method finding exact matches divided by total number of common
	 * children
	 *
	 * @return the scoring function for Jaccard
	 */
	static ScoringFunction getJaccardScore() {
		return (match, childScores) -> new Score(
				(double) childScores.size() / ((match.getData().getChildCount(match.getMatchedWith()))), match);
	}
	...
	static List<Score> getNonPerfectMatchedElement(List<Score> childScoreList) {
		return childScoreList.stream().filter(d -> d.getResult() < EXPONENTIAL_INCREASE_THRESHOLD)
