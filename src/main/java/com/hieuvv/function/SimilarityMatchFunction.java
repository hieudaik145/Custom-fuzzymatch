...
import com.hieuvv.exception.MatchException;
...
public interface SimilarityMatchFunction extends BiFunction<Token, Token, Double> {
	...
	static SimilarityMatchFunction levenshtein() {
		return (leftToken, rightToken) -> {
			...
			return (maxLength - StringUtils.getLevenshteinDistance(left.toLowerCase(), right.toLowerCase()))
					/ maxLength;
		};
	}
	...
	static SimilarityMatchFunction phoneNumber() {
		return (left, right) -> {
			PhoneNumberUtil.MatchType output = PhoneNumberUtil.getInstance().isNumberMatch(left.getValue().toString(),
					right.getValue().toString());
			return (output == PhoneNumberUtil.MatchType.NSN_MATCH || output == PhoneNumberUtil.MatchType.EXACT_MATCH
	...
	static SimilarityMatchFunction numberDifferenceRate() {
		return (left, right) -> {
			...
			Double leftValue = NumberUtils.isNumber(left.getValue().toString())
					? Double.valueOf(left.getValue().toString())
					: Double.NaN;
			Double rightValue = NumberUtils.isNumber(right.getValue().toString())
					? Double.valueOf(right.getValue().toString())
					: Double.NaN;
